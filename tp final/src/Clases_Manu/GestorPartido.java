...
public class GestorPartido implements MetodosComunes<Partido, String> {
    ...
    public GestorPartido(GestionJugadores gestionJugadores) {
        this.partidos = new ArrayList<>();
        this.gestionJugadores = gestionJugadores;
    }

    public void agregarPartido(String fecha, boolean esLocal, String rival, int golesAFavor, int golesEnContra,
                               int entradasVendidas, double precioEntrada, GestionPresupuesto gestionPresupuesto) throws IngresoInvalido, ElementoDuplicadoEx {
        if (fecha == null || fecha.isEmpty()) throw new IngresoInvalido("La fecha del partido no puede estar vacía.");
        boolean existe = partidos.stream().anyMatch(p -> p.getFecha().equals(fecha));
        if (existe) throw new ElementoDuplicadoEx("Ya existe un partido registrado en la fecha " + fecha);
    ...
    public boolean existe(String fecha) throws ElementoInexistenteEx {
        boolean existe = partidos.stream().anyMatch(p -> p.getFecha().equals(fecha));
        if (!existe) throw new ElementoInexistenteEx("No se encontró un partido con la fecha " + fecha);
        return true;
    }
